...
##### 반복 검색
- `{m,n}`은 `앞선 패턴`(다음 예제의 경우 A)이 `최소 m번, 최대 n번 반복되는 문자열`을 의미한다.
...
```
- `{n,}`는 `앞선 패턴이 최소 n번 이상 반복되는 문자열`을 의미한다.
```javascript
const target = 'A AA B BB Aa Bb AAA';
...
```
- `+`는 `앞선 패턴이 최소 한 번 이상 반복되는 문자열`을 의미한다.
- 즉, `+`는 `{1,}`과 같다.
```javascript
const target = 'A AA B BB Aa Bb AAA';
...
```
- `?`는 `앞선 패턴이 최대 한 번(0번 포함) 이상 반복되는 문자열`을 의미한다.
- 즉, `?`는 `{0,1}`과 같다.
```javascript
...
- `\s`는 여러 가지 `공백 문자(스페이스, 탭 등)를 의미`한다.
- 즉, `\s`는 `[\t\r\n\v\f]`와 같은 의미이다.
```javascript
const target = ' Hi!';
...
##### 한글만으로 이루어진 문자열(공백포함)
```javascript
const target = '김지원 입니다'
const korean = /^[ㄱ-ㅎ|ㅏ-ㅣ|가-힣\s]+$/g;

korean.test(target); // true
```

...
- 반환문의 2가지 역할
	...
	2. 반환문은 return 키워드 뒤에 오는 표현식을 평가해 반환한다. return 키워드 뒤에 반환값으로 사용할 표현식을 명시적으로 지정하지 않으면 `undefined`가 반환된	다.
```javascript
function foo () {
	return;
}
...
- `고차함수(Higher-Order Function, HOF)` : 매개변수를 통해 함수의 외부에서 콜백함수를 전달받은 함수, 매개변수를 통해 함수를 전달받거나 반환값으로 함수를 반환하는 함수

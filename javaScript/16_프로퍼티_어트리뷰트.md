...
- 내부 슬롯과 내부 메서드는 자바스크립트 엔진의 내부 로직이므로 원칙적으로 자바스크립트는 내부 슬롯과 내부 메서드에 `직접 접근하거나 호출할 방법을 제공하지 않는다.` → 정보 은닉
- 정보 은닉은 좋은 의미이다. 정보 은닉을 하지 않으면 개발자가 그 정보를 다 알아야한다.
- 단, 일부 내부 슬롯과 내부 메서드에 한하여 `간접적으로` 접근할 수 있는 수단을 제공하기는 한다.
- 예를 들어, 모든 객체는 `[[Prototype]]`이라는 내부 슬롯을 갖는다. 내부 슬롯은 자바스크립트 엔진의 내부 로직이므로 원칙적으로 직접 접근할 수는 없지만, `[[Prototype]]` 내부 슬롯의 경우 `__proto__`과 `Object.getOwnPrototypeOf(식별자);`를 통해 간접적으로 접근할 수 있다.
...
console.log(hi.x); // 1
console.log(hi.getX()); // 1
```
- 두 번째 방법은 함수를 호출해야하지만, 접근자 프로퍼티를 사용하면 표현식처럼 사용이 가능하다.
- 2가지 방법 모두 사용가능하자만, 1번째 방법을 추천한다. 

```javascript
console.log(document.querySelector('ul').children);
// li들이 나옴, children은 접근자 프로퍼티
```
- 접근자 프로퍼티는 표현식처럼 사용하기 편리하다.
- children은 빌트인 프로퍼티이다.
...
*/
```
- getter 또는 setter 두가지 중 한가지만 가질 수 있다.(set: undefined)

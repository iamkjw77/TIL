...
- 따라서 constructor 내부에서 return 문읇 반드시 생략해야 한다.
...
- super가 호출되면 수퍼클래스의 contructor(super-constructor)가 호출된다.
...
- 따라서 생성된 인스턴스의 프로토타입은 수퍼클래스의 prototype 프로퍼티가 가리키는 객체(Rectangle.prototype)가 아니라 new.target, 즉 서브클래스의 prototype 프로퍼티다 가리키는 객체(ColorRectangle.prototype)이다.
```javascript
class Rectangle {
  constructor(width, height) {
    // 암묵적으로 빈 객체, 즉 인스턴스가 생성되고 this에 바인딩된다.

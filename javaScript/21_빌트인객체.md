## JAVASCRIPT
<details open>
<summary>빌트인 객체</summary>
<div markdown="21">

#### 자바스크립트 객체의 분류
- 자바스크립트 객체는 다음과 같이 크게 3개의 객체로 분류할 수 있다.

1. `표준 빌트인 객체(standard built-in object / native object / global object)`
- 표준 빌트인 객체는 `ECMAScript 사양에서 정의된 객체`를 말하며, 애플리케이션 전역의 공통 기능을 제공한다.
- 표준 빌트인 객체는 ECMAScript 사양에서 정의된 객체이므로 `자바스크립트 실행환경(브라우저 또는 Node.js 환경)과 관계없이 언제나 사용할 수 있다.`
- 따라서 별도의 선언이 필요없다.
- 표준 빌트인 객체는 전역 객체의 프로퍼티로서 제공된다.
- 따라서 별도의 선언 없이 전역 변수처럼 언제나 참조할 수 있다.
- ex) window.Object === Object // true, Object는 생성자 함수이면서 객체이다.
- 브라우저 탭을 생성 → 전역 객체를 만듦(빌트인 객체들이 다 만들어진다.)

2. `호스트 객체(host object)`
- 호스트 객체는 `ECMAScript 사양에 정의되어 있지 않지만,` `자바스크립트 실행 환경(브라우저 환경 또는 Node.js 환경)에서 추가로 제공하는 객체`를 말한다.
- 브라우저 환경에서는 DOM, BOM, Canvas, XMLHttpRequest, fetch, reqestAnimationFrame, SVG, Web Strorage, Web Component, Web worker와 같은 클라이언트 사이드 Web API를 호스트 객체로 제공하고 Node.js 환경에서는 Node.js 고유의 API를 호스트 객체로 제공한다.
...
- `생성자 함수 객체인 표준 빌트인 객체는 프로토타입 메서드와 정적 메소드를 제공`하고, `생성자 함수 객체가 아닌 표준 빌트인 객체는 정적 메소드만 제공`한다.
...
const date = new Date(); // 2020-12-09T20:22:18.713Z
...
```
- 표준 빌트인 객체의 prototype 프로퍼티의 바인딩된 객체는 `다양한 기능의 빌트인 프로토타입 메서드를 제공`한다.
- 그리고 `표준 빌트인 객체는 인스턴스 없이도 호출 가능한 빌트인 정적 메서드를 제공`한다.
```javascript
...
- 즉, `원시값을 객체처럼 사용`하면 자바스크립트 엔진이 암묵적으로 연관된 객체를 생성하여 생성된 객체로 프로퍼티에 접근하거나 메서드를 호출하고 다시 원시값으로 되돌린다.
- 이처럼 `문자열, 숫자, 불리언 값에 대해 객체처럼 생성되는 임시 객체를 래퍼 객체(wrapper object)`라 한다.
...
![문자열 래퍼 객체의 프로토타입 체인](../images/wrapper_object.PNG)
...
str.name = 'Lee';
// str은 원래의 문자열, 즉 래퍼 객체의 [[StringData]] 내부 슬롯에 할당된 원시값을 갖는다.
// 이때 위에서 생성한 래퍼객체는 아무도 참조하지 않는 상태이므로 가비지 컬렉션의 대상이 된다.
...
- 이처럼 `문자열, 숫자, 불리언, 심벌은 암묵적으로 생성되는 래퍼 객체에 의해 마치 객체처럼 사용`할 수 있으며, 표준 빌트인 객체인 String, Number, Boolean, Symbol의 프로토타입 메서드 또는 프로퍼티를 참조할 수 있다.
- 따라서 `String, Number, Boolean 생성자 함수를 new 연산자와 함께 호출하여 문자열, 숫자, 불리언 인스턴스를 생성할 필요가 없으며 권장하지도 않는다.`
- Symbol은 생성자 함수가 아니므로 이 논의에서 제외하도록 한다.

- 문자열, 숫자, 불리언, 심벌 이외의 원시값, 즉 null과 undefined는 래퍼 객체를 생성하지 않는다.
- 따라서 `null과 undefined 값을 객체처럼 사용하면 에러가 발생`한다.

#### 전역 객체(global object)
- 전역 객체는 `코드가 실행되기 이전 단계에 자바스크립트 엔진에 의해 어떤 객체보다도 먼저 생성되는 특수한 객체`이며, `어떤 객체에도 속하지 않는 최상위 객체`이다.
...
5. `let이나 const 키워드`로 선언한 전역 변수는 전역 객체의 프로퍼티가 아니다. 즉, window.foo와 같이 접근할 수 없다. let이나 const 키워드로 선언한 변수는 보이지 않는 개념적인 블록 내에 존재하게 된다.
```javascript
let foo = 123;
console.log(window.foo); // undefined
```
5. 브라우저 환경의 모든 자바스크립트 코드는 하나의 전역 객체 window를 공유한다. 여러 개의 script 태그를 통해 자바스크립트 코드를 분리해도 하나의 전역 객체 window를 공유하는 것은 변함이 없다. 이는 분리되어 있는 자바스크립트 코드가 하나의 전역을 공유한다는 의미이다.
...
2. `빌트인 전역 함수(built-in global function)`는 애플리케이션 전역에서 호출할 수 있는 함수로서 전역 객체의 메서드이다.

##### eval
- eval 함수는 자바스크립트 코드를 나타내는 문자열을 인수로 받는다.
- 전달받은 문자열 코드가 표현식이라면 eval 함수는 문자열 코드를 런타임에 평가하여 값을 생성하고, 전달받은 인수가 표현식이 아닌 문이라면 eval 함수는 문자열 코드를 런타임에 실행한다. 문자열 코드가 여러 개인 문으로 이루어져 있다면 모든 문을 실행한다.

```javascript
// code – 코드를 나타내는 문자열
// 문자열 코드를 평가/실행한 결과값을 return
eval(code)
```
...
- 인수로 전달받은 문자열 코드가 여러 개인 문으로 이루어져 있다면 모든 문을 실행한 다음, 마지막 결과값을 반환한다.
```javascript
...
- 또한 `let, const 키워드를 사용한 변수 선언문이라면 암묵적으로 strict mode가 적용`된다.
```javascript
const x = 1;
...
console.log(x); // 1
```
- eval 함수를 통해 사용자로부터 입력받은 콘텐츠(undefined data)를 실행하는 것은 보안에 매우 취약하다.
...
- `NaN === NaN // false` NaN은 자신과 다른 유일한 값이다.
- 표준 빌트인 객체 isNaN은 숫자가 아닌 경우 숫자로 타입을 변환한다.(초창기에 만들어짐)
- 사실 isNaN은 `Number.isNaN`이 존재한다.(Number 정적 메서드로 최신이다, 무조건 숫자를 인수로 주어야한다, 더 엄격하다.)
- 따라서 `Number.isNaN`을 사용하자
```javascript
// testValue – 검사 대상 값
// NaN 여부 확인 결과 return
isNaN(testValue)
...
isNaN('blablabla'); // true: 'blablabla' => NaN
isNaN('10'); // false
isNaN('10.12'); // false
isNaN(''); // false: '' => 0
isNaN(' '); // false: ' ' => 0
...
##### parseFloat
- 전달받은 문자열 인수를 부동 소수점 숫자(floating point number), 즉 `실수로 해석(parsing)하여 반환`한다.
```javascript
// string – 변환 대상 값
// 변환 결과(number) return
parseFloat(string)
...
##### parseInt
- 전달받은 문자열 인수를 `정수(integer)로 해석(parsing)하여 반환`한다.
```javascript
// string – 변환 대상의 값
...
parseInt('10', 16); // 16
```
- 참고로 기수를 지정하여 10진수 숫자를 해당 기수의 문자열로 변환하여 반환하고 싶을 때는 `Number.prototype.toString 메서드를 사용`한다.
```javascript
const x = 15;
...
```
- 두 번째 인수로 진법을 나타내는 기수를 지정하지 않더라도 첫 번째 인수로 전달된 문자열이 “0x” 또는 “0X”로 시작하는 16진수 리터럴이라면 16진수로 해석하여 10진수로 반환한다.
```javascript
parseInt('0xf'); // 15
parseInt('f', 15); // 15
```
...
// 8진수 리터럴(ES6에서 도입, 0x로 시작)은 제대로 해석하지 못한다. 0 이후가 무시된다.
parseInt('0o10'); // 0
```
...
##### encodeURI / decodeURI
- encodeURI 함수는 완전한 URI(Uniform Resource Identifier)를 문자열로 전달받아 `이스케이프 처리를 위해 인코딩`한다.
...
```javascript
// uri – 완전한 URI
// 인코딩된 URI(string)을 return
encodeURI(uri)
```

- `인코딩 : URI의 문자들을 이스케이프 처리하는 것을 의미`
- 이스케이프 처리 : 네트워크를 통해 정보를 공유할 때 어떤 시스템에서도 읽을 수 있는 `아스키 문자 셋(ASCII Character-set)으로 변환하는 것`
- UTF-8 특수 문자의 경우 1문자당 1~3바이트, UTF-8 한글 표현의 경우 1문자당 3바이트이다.
...
- 따라서 URL 내에서 의미가 있는 문자(`%, ?, #`)나 URL에 올 수 없는 문자(`한글, 공백 등`) 또는 시스템에 의해 해석될 수 있는 문자(`<,>`)를 이스케이프 처리하여 야기될 수 있는 문제를 예방하기 위해 이스케이프 처리가 필요하다.
...
- decodeURI 함수는 인코딩된 URI를 인수로 전달받아 `이스케이프 처리 이전으로 디코딩`한다.
```javascript
// encodedURI – 인코딩된 URI
// 디코딩된 URI(string) return
...
- encodeURIComponent 함수는 `URI 구성요소(component)를 인코딩`한다.
- decodeURIComponent 함수는 매개변수로 전달된 `URI 구성요소를 디코딩`한다.
```javascript
// uriComponent – URI의 구성요소
// 인코딩된 URI의 구성요소(string) return
encodeURIComponent(uriComponent)
```
```javascript
// encodedURIComponent – 인코딩된 URI의 구성요소
// 디코딩된 URI의 구성요소(string) return 
decodeURIComponent(encodedURIComponent)
```

- `encodedURIComponent 함수`는 인수로 전달된 문자열을 URI의 구성요소인 쿼리 스트링의 일부로 간주한다.
- 따라서 `쿼리 스트링 구분자로 사용되는 =, ?, & 까지 인코딩`한다.

- 반면 `encodeURI 함수`는 매개변수로 전달된 문자열을 완전한 URI 전체라고 간주한다.
...
- 위의 예제에서 자바스크립트 엔진은 `y = 20`을 `window.y = 20`으로 해석하여 전역 객체의 프로퍼티를 동적 생성한다.

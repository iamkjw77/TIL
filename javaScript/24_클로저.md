...
- 일급 객체가 존재해야 함수형 프로그래밍이 가능하고, 함수형 프로그래밍의 대표적인 예가 클로저이다.
...
function outer() {
  ...
  return inner;
  // inner 함수 객체의 참조값을 return
  // 함수가 함수를 반환
}

const innerFunc = outer();
innerFunc(); // 10, 클로저의 개념이 없다면 1이 나와야한다.
```
- 중첩함수를 사용하는 이유 : 중첩함수는 외부함수의 일부분을 추상화한 것이다. 중첩함수를 추상화하여 재사용하고, 함수이름을 부여함으로서 가독성이 좋아진다.
- 자바스크립트에서 중첩함수는 클로저와 관련이 있다.
- 클로저를 만들기 위해서 중첩함수를 만드는 경우도 있다.
...
- 일반적으로 지역변수의 생명주기는 함수의 생명주기와 일치한다.
...
- `[[Environment]]` 내부 슬롯에 할당된 참조값으로 상위스코프를 기억하기 때문이다.
- 따라서 함수는 언제나 2개의 스코프를 가진다.(자기 자신의 스코프, 상위스코프)
...
- 중첩함수 bar를 반환한 경우라도 외부코드에서 중첩함수를 받아주어야한다.(ex) 전역 변수에 할당)
- 외부함수가 중첩함수를 반환하는 경우라도 외부에서 중첩함수를 받아주지 않으면 클로저가 아니다.  
...
- 메모리 위기 : 할 일을 다한 객체들이 메모리를 점유하고 있는 경우
- 자바스크립트는 개발자가 직접 메모리를 관리해야 하는 것이 아니라 매니지드 언어이기 때문에 자바스크립트 엔진이 메모리를 관리해준다.(가비지 컬렉터가 메모리를 관리)
...
- 위에서의 상태의 예시로는 상태변수 count 같이 상태가 변하는 변수가 있다.
...
// 외부 상태를 변경하므로 비순수 함수
// num은 increase 함수만 변경해야한다는 전제가 필요하다.
...
// 1, 즉시실행함수가 아니기 때문에 호출할 때마다 increase함수가 계속 호출됨
console.log(increase()()); // 1
console.log(increase()()); // 1
```
- 클로저는 기본적으로 하나의 렉시컬환경을 만들기 위해서 `즉시 실행 함수로 감싼다.`
...
- 위 예제에서는 렉시컬 환경이 2번 만들어진다.(makeCounter를 2번 호출했기 때문에)
...
- class를 쓰면 접근 제한자 private를 일부 사용할 수 있다.
- 따라서 객체 리터럴, 생성자 함수는 기본적으로 정보은닉이 안된다.
...
// 하나의 렉시컬 환경을 만들기 때문에 _age의 값이 30이 나온다.
// private를 쓰고 싶다면 class의 private 필드 제안을 쓰는 방법 밖에 없다.
```
...
// i는 var로 선언되었기 때문에 전역변수이다.
// function () { return i; }; 의 상위스코프는 전역 스코프이다.
// 그래서 funcs[j]를 호출할 때, i의 값은 3이기 때문에 333이 나온다. 
```
...
// 순차적으로 만들어진 렉시컬 환경의 상위스코프는 바로 앞순서에서 만들어진 렉시컬 환경이다.
// for문 안에 let, const 키워드로 선언한 변수만 블록 렉시컬 환경을 만든다.(var 키워드는 안만듦)
```
- for 문의 변수 선언문에서 let 키워드로 선언한 변수를 사용하면 for 문의 코드 블록이 반복 실행될 때마다 for 문 코드 블록의 새로운 렉시컬 환경이 생성된다.
- for 코드 블록은 소스코드 타입 4가지에 들어가지 않으므로 실행 컨텍스트를 만들지 않는다.
